...
// Function to get the previous trading day's timestamp
const getPreviousTradingDayTimestamp = () => {
    ...
    return Math.floor(prevDate.getTime() / 1000); // Convert to UNIX timestamp
};
...
app.get("/stocks", async (req, res) => {
    const results = await Promise.all(stockSymbols.map(fetchStockData));
    const filteredResults = results.filter(stock => stock !== null);
